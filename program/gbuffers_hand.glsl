...
float GetHandItem(int id) {
	return float((heldId == id && isMainHand > 0.5) || (heldId2 == id && isMainHand < 0.5));
}
...
void main() {
	...
	if (albedo.a > 0.001) {
		...
		#endif
		lightmap.x = max(lightmap.x, GetHandItem(3));

		float emissive = (GetHandItem(1) + GetHandItem(2) + GetHandItem(3));
		...
		emission *= GetHardcodedEmission(albedo.rgb, hsv) * 0.5;
		...
		float doRecolor = GetHandItem(2) + GetHandItem(3);
		...
		#if defined MULTICOLORED_BLOCKLIGHT || defined MCBL_SS
		blocklightCol = ApplyMultiColoredBlocklight(blocklightCol, screenPos, vec3(0.0), newNormal);
		#endif
...
void main() {
	...
	#if MC_VERSION >= 11500
	#ifdef IS_IRIS
	isMainHand = float(gl_Position.x * (isRightHanded ? 1 : -1) > 0.0);
	#else
