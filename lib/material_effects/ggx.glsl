...
float EminGTSquarePhong(float sunSize, vec3 normal, vec3 lightVec, vec3 viewPos, float roughness) {
...
vec3 GGX(vec3 normal, vec3 viewPos, float smoothness, vec3 baseReflectance, float sunSize) {
    ...
    #if SHADER_SUN_MOON_SHAPE_INTERNAL == 0
    float D = GGXTrowbridgeReitz(NoHsqr, roughness);
    #else
    float D = EminGTSquarePhong(sunSize, normal, lightVec, viewPos, roughness);
    #endif
