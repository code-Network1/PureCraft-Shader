#if TAA_MODE == 1
    // EminGT's TAA Mode 1: Balanced settings for optimal quality/performance
...
#elif TAA_MODE == 2
    // EminGT's TAA Mode 2: High quality settings for premium systems
...
#elif TAA_MODE >= 3
    // Ultra Quality TAA (TAA x8 Equivalent) - Heavy
    float blendMinimum = 0.85;   // Very strong accumulation
    float blendVariable = 0.1;   // Less temporal tearing
    float blendConstant = 0.85;  // Preserves deep clarity
    float regularEdge = 1.5;     // Highly sensitive micro-edge detection
    float extraEdgeMult = 6.0;   // Powerful edge clamp
#endif
...
// EminGT's motion vector reprojection for temporal consistency
...
// EminGT's AABB color clamping for temporal artifact prevention
