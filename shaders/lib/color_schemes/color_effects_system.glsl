...
// Light & Ambient Multipliers
    ...
    // Atmospheric color multiplier calculator
    vec3 GetAtmColorMult() {
        vec3 atmColorMult;

        #ifdef OVERWORLD
            // Atmospheric color temperature
...
// Moon Phase Influence
    ...
    #ifdef OVERWORLD
        // Moon phase lighting calculator
        float moonPhaseInfluence = mix(
            1.0,
            moonPhase == 0 ? MOON_PHASE_FULL :
            moonPhase != 4 ? MOON_PHASE_PARTIAL :
            MOON_PHASE_DARK,
            1.0 - sunVisibility2
        );
    #else
