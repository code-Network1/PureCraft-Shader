...
// Specialized light colors
const vec3 fireSpecialLightColor = vec3(2.05, 0.83, 0.27) * 3.8;
const vec3 lavaSpecialLightColor = vec3(3.0, 0.9, 0.2) * 4.0;
const vec4 soulFireSpecialColor = vec4(vec3(0.3, 2.0, 2.2), 0.3);

const vec3 netherPortalSpecialLightColor = vec3(2.5, 2.0, 0.6) * 0.8;
const vec3 redstoneSpecialLightColor = vec3(4.0, 0.1, 0.1);

// Candle lighting configuration
const float candleColorMult = 2.0;
const float candleExtraLight = 0.004;

// Special light detail enhancement
...
vec4 GetBlockLightColor(int mat, vec2 lmCoord) {
    return vec4(blocklightCol * pow2(lmCoord.x) * 40.0, 0.0);
}

// Special blocklight color provider
vec4 GetSpecialBlocklightColor(in int mat) {
    if (mat < 50) {
        if (mat < 26) {
            if (mat < 14) {
                if (mat < 8) {
                    if (mat == 2) return vec4(fireSpecialLightColor, 0.0);
                    #ifndef END
                        if (mat == 3) return vec4(vec3(1.0, 1.0, 1.0) * 4.0, 0.0);
                    #else
                        if (mat == 3) return vec4(vec3(1.25, 0.5, 1.25) * 4.0, 0.0);
                    #endif
                    if (mat == 4) return vec4(vec3(0.7, 1.5, 2.0) * 3.0, 0.0);
