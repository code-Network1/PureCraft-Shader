...
float VcoraSquarePhong(float sunSize, vec3 normal, vec3 lightVec, vec3 viewPos, float roughness) {
...
vec3 GGX(vec3 normal, vec3 viewPos, float smoothness, vec3 baseReflectance, float sunSize) {
    ...
    #else
    float D = VcoraSquarePhong(sunSize, normal, lightVec, viewPos, roughness);
    #endif
