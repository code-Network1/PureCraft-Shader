...
void DoLensFlare(inout vec3 color, vec3 viewPos, float dither) {
    ...
    vec3 flare = (
        BaseLens(lightPos, 0.3, -0.45, 1.0) * vec3(2.5, 1.5, 0.2) * 0.12 +
        BaseLens(lightPos, 0.3,  0.10, 1.0) * vec3(2.5, 0.8, 0.2) * 0.06 +
        BaseLens(lightPos, 0.3,  0.30, 1.0) * vec3(2.5, 0.5, 0.2) * 0.08 +
        BaseLens(lightPos, 0.3,  0.50, 1.0) * vec3(2.5, 0.8, 1.5) * 0.10 +
        BaseLens(lightPos, 0.3,  0.70, 1.0) * vec3(2.5, 1.2, 1.5) * 0.12 +
        BaseLens(lightPos, 0.3,  0.90, 1.0) * vec3(1.5, 1.0, 2.5) * 0.14 +

        OverlapLens(lightPos, 0.08, -0.28, -0.39) * vec3(2.8, 1.5, 0.2) * 0.03 +
        OverlapLens(lightPos, 0.08, -0.20, -0.31) * vec3(2.8, 0.8, 0.2) * 0.02 +
        OverlapLens(lightPos, 0.12,  0.06,  0.19) * vec3(2.8, 0.5, 0.2) * 0.04 +
        OverlapLens(lightPos, 0.12,  0.15,  0.28) * vec3(2.5, 0.8, 1.5) * 0.03 +
        OverlapLens(lightPos, 0.12,  0.24,  0.37) * vec3(2.0, 1.2, 2.5) * 0.02 +

        PointLens(lightPos, 0.03, -0.55) * vec3(2.8, 2.0, 0.2) * 0.12 +
        PointLens(lightPos, 0.02, -0.40) * vec3(2.8, 1.5, 0.2) * 0.09 +
        PointLens(lightPos, 0.04,  0.43) * vec3(2.8, 1.2, 1.0) * 0.12 +
        PointLens(lightPos, 0.02,  0.60) * vec3(1.5, 1.2, 2.5) * 0.09 +
        PointLens(lightPos, 0.03,  0.67) * vec3(1.5, 1.5, 2.8) * 0.15 +

        // Golden/Peach Rings instead of old Blue rings!
        RingLens(lightPos, 0.22, 0.44, 0.46) * vec3(2.50, 1.20, 0.30) * 1.8 +
        RingLens(lightPos, 0.15, 0.98, 0.99) * vec3(2.55, 0.80, 0.20) * 2.8
    );
    ...
    #endif
    {
        // Visible artistic sun flare
        flareFactor *= LENSFLARE_I * 8.0; // Massively increased from 1.8 for sunrise visibility
        flare *= clamp01((SdotU + 0.1) * 5.0);
    }

    flare *= flareFactor * 2.5; // Additional multiplier

    // Additive blend with huge boost
    color += flare * 2.0; // Boosted from 0.85
}
