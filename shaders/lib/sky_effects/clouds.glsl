...
float GetCloudMask(vec3 viewPos, float z) {
	...
	// Disabled FAR_VANILLA_FOG for better cloud visibility
	return 1.0;
}
...
vec4 DrawCloudSkybox(vec3 viewPos, float z, float dither, vec3 lightCol, vec3 ambientCol, bool fadeFaster) {
	...
	if (VoU > 0.025) {
		...
		// Enhanced skybox cloud coloring
		vec3 baseAmbient = ambientCol * (0.3 * sunSkyVisibility + 0.5);
		vec3 baseLighting = lightCol * (0.85 + 1.15 * scattering);
		
		// Add time-based color variations for skybox clouds
		float timeProgress = timeAngle * 2.0;
		vec3 skyboxTimeCol;
		
		if (timeProgress < 0.25) { // Dawn
			skyboxTimeCol = vec3(1.0, 0.7, 0.4);
		} else if (timeProgress < 0.5) { // Day
			skyboxTimeCol = vec3(1.0, 0.98, 0.95);
		} else if (timeProgress < 0.75) { // Dusk
			skyboxTimeCol = vec3(1.1, 0.5, 0.3);
		} else { // Night
			skyboxTimeCol = vec3(0.4, 0.5, 0.7);
		}
		
		baseLighting = mix(baseLighting, skyboxTimeCol * baseLighting, 0.3);
		
		cloudColor = mix(baseAmbient, baseLighting, cloudLighting);
...
vec4 DrawCloudVolumetric(vec3 viewPos, vec3 cameraPos, float z, float dither, vec3 lightCol, vec3 ambientCol, inout float cloudViewLength, bool fadeFaster) {
	...
	float fadeStart = 128.0 / max(fogDensity, 0.5);
	float fadeEnd = (fadeFaster ? 320.0 : 960.0) / max(fogDensity, 0.5);

	float xzNormalizeFactor = 5.0 / max(abs(float(CLOUD_HEIGHT) - 72.0), 56.0);
	...
	// Enhanced cloud coloring based on time of day
	vec3 cloudAmbientCol = ambientCol * (0.3 * sunSkyVisibility + 0.5);
	
	#ifdef PURECRAFT_CLOUD_COLORS
	// Sunrise/Sunset colors
	vec3 sunriseCol = vec3(1.0, 0.9, 0.8) * sunSkyVisibility;
	vec3 sunsetCol = vec3(1.0, 0.8, 0.7) * sunSkyVisibility;
	vec3 noonCol = vec3(1.0, 1.0, 0.98) * sunSkyVisibility;
	
	// Enhanced moon phase colors
	float moonPhaseIntensity[8] = float[8](1.0, 0.9, 0.7, 0.5, 0.3, 0.5, 0.7, 0.9);
	float moonIntensity = moonPhaseIntensity[moonPhase] * moonVisibility;
	vec3 nightCol = mix(vec3(0.1, 0.15, 0.3), vec3(0.4, 0.5, 0.7), moonIntensity);
	...
	if (timeProgress < 0.25) { // Dawn
		timeBasedCol = mix(nightCol, sunriseCol, timeProgress * 4.0);
	} else if (timeProgress < 0.5) { // Morning to Noon
		timeBasedCol = mix(sunriseCol, noonCol, (timeProgress - 0.25) * 4.0);
	} else if (timeProgress < 0.75) { // Afternoon to Evening
		timeBasedCol = mix(noonCol, sunsetCol, (timeProgress - 0.5) * 4.0);
	} else { // Night
		timeBasedCol = mix(sunsetCol, nightCol, (timeProgress - 0.75) * 4.0);
	}
	
	vec3 cloudLightCol = mix(lightCol, horizonSunCol, horizonSunFactor);
	cloudLightCol = mix(cloudLightCol, timeBasedCol, 0.4); // Blend with time-based colors
	#else
	vec3 cloudLightCol = mix(lightCol, horizonSunCol, horizonSunFactor);
	#endif
	...
	// Weather-based color adjustments with storm colors
	#ifdef PURECRAFT_CLOUD_COLORS
	vec3 stormCol = vec3(0.4, 0.45, 0.6); // Dark bluish storm clouds
	cloudColor = mix(cloudColor, stormCol, rainStrength * 0.6);
	cloudColor *= 1.0 - 0.3 * rainStrength; // Less harsh darkening
	
	// Enhanced brightness with color temperature
	float colorTemp = mix(0.95, 1.05, sunSkyVisibility); // Much less temperature variation
	cloudColor.r *= colorTemp;
	cloudColor.g *= mix(colorTemp, 1.0, 0.8); // Less green tinting
	cloudColor.b *= mix(1.05, colorTemp * 0.98, sunSkyVisibility); // Subtle blue adjustment
	
	// Add subtle atmospheric scattering effect
	vec3 scatterCol = vec3(0.6, 0.8, 1.0) * scattering * 0.3;
	cloudColor += scatterCol * sunSkyVisibility;
	#else
	cloudColor *= 1.0 - 0.4 * rainStrength;
	#endif
...
#if AURORA > 0
#include "../color_grading/auroraColor.glsl"
