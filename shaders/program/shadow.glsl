        ...
        if (mat < 32008) {
            if (mat < 32000) {
                ...
                if (mat == 32000) { // Water
                    ...
                    // Water Caustics - Enhanced Golden
                    #if WATER_CAUSTIC_STYLE < 3
                        #if MC_VERSION >= 11300
                            float wcl = GetLuminance(color1.rgb);
                            color1.rgb = color1.rgb * pow2(wcl) * 1.2;
                            // Add golden tint to caustics
                            color1.rgb *= vec3(1.2, 1.1, 0.9);
                        #else
                            color1.rgb = mix(color1.rgb, vec3(GetLuminance(color1.rgb)), 0.88);
                            color1.rgb = pow2(color1.rgb) * vec3(3.0, 2.8, 2.2) * 0.96; // More golden caustics
                        #endif
                    #else
