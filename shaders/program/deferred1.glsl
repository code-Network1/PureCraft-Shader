...
float GetAmbientOcclusion(float z){
	float ao = 0.0;
	float tw = 0.0;
	float lz = GetLinearDepth(z, gbufferProjectionInverse);
	
	for(int i = 0; i < 4; i++){
		vec2 sampleOffset = aoSampleOffsets[i] / vec2(viewWidth, viewHeight);
		vec2 depthOffset = aoDepthOffsets[i] / vec2(viewWidth, viewHeight);
		float samplez = GetLinearDepth(texture2D(depthtex0, texCoord + depthOffset).r, gbufferProjectionInverse);
...
#include "../lib/sky_effects/sunmoon.glsl"

#ifdef OUTLINE_ENABLED
#include "../lib/utility_functions/outlineOffset.glsl"
#include "../lib/utility_functions/outlineDepth.glsl"
#include "../lib/sky_effects/waterFog.glsl"
#include "../lib/post_processing/outline.glsl"
#endif

#if defined ADVANCED_MATERIALS && defined REFLECTION_SPECULAR
#include "../lib/utility_functions/encode.glsl"
#include "../lib/mirror_effects/raytrace.glsl"
#include "../lib/mirror_effects/complexFresnel.glsl"
#include "../lib/material_effects/materialDeferred.glsl"
#include "../lib/mirror_effects/roughReflections.glsl"
#endif
...
void main() {
	...
	Outline(color.rgb, viewPos.xyz, false, outerOutline, innerOutline, outlineLinZ);
