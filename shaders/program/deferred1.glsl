...
#if AURORA_STYLE > 0 || defined NIGHT_NEBULA || defined END
...
// Include effects_unified for various effects
#if defined WORLD_OUTLINE || defined DARK_OUTLINE
...
void main() {
    ...
    if (z0 < 1.0) {
        ...
        if (materialMaskInt <= 240) {
            #ifdef IPBR
                #include "/lib/materialHandling/deferredMaterials.glsl"
        ...
        color.rgb *= ssao;
            ...
            float fresnelM = max(fresnel - fresnelFactor, 0.0) / (1.0 - fresnelFactor);
                ...
                if (materialMaskInt == 1) fresnelM = 0.65; // Permanent steady reflection for puddles
            #else
